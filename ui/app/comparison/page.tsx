...
import { ArrowRightLeft, CreditCard, Download, Scale, ShieldAlert, Zap, Loader2 } from "lucide-react"
...
export default function ComparisonPage() {
    ...
    const handleExportPDF = () => {
        ...
        const lines: string[] = [
            '═══════════════════════════════════════════════',
            '       CREDITRUST CROSS-PRODUCT ANALYSIS',
            ...
            '───────────────────────────────────────────────',
            `  BASELINE: ${productA}`,
            `  Total Complaints: ${comparisonData.productA.total.toLocaleString()}`,
            '───────────────────────────────────────────────',
            '',
            '  Top Issues:',
        ]
        comparisonData.productA.topIssues.forEach((issue: any, i: number) => {
            const pct = Math.round((issue.count / comparisonData.productA.total) * 100)
            lines.push(`    ${i + 1}. ${issue.issue} — ${pct}%`)
        })
        lines.push('')
        lines.push('  Top Companies:')
        comparisonData.productA.topCompanies.forEach((c: any) => {
            lines.push(`    • ${c.company}`)
        })
        
        lines.push('')
        lines.push('───────────────────────────────────────────────')
        lines.push(`  COMPARISON: ${productB}`)
        lines.push(`  Total Complaints: ${comparisonData.productB.total.toLocaleString()}`)
        lines.push('───────────────────────────────────────────────')
        lines.push('')
        lines.push('  Top Issues:')
        comparisonData.productB.topIssues.forEach((issue: any, i: number) => {
            const pct = Math.round((issue.count / comparisonData.productB.total) * 100)
            lines.push(`    ${i + 1}. ${issue.issue} — ${pct}%`)
        })
        lines.push('')
        lines.push('  Top Companies:')
        comparisonData.productB.topCompanies.forEach((c: any) => {
            lines.push(`    • ${c.company}`)
        })
        lines.push('')
        lines.push('═══════════════════════════════════════════════')
        lines.push('  Report generated by CrediTrust Intelligence')
        lines.push('═══════════════════════════════════════════════')
        ...
        a.href = url
        a.download = `CrediTrust_Comparison_${productA}_vs_${productB}_${new Date().toISOString().split('T')[0]}.txt`
        a.click()
